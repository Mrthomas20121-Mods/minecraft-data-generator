...
manager.run((assets, data, modid) => {
    let metalParts = load('./src/datagen/data/metal_parts.json');
    let toolMetalParts = load('./src/datagen/data/tool_metal_parts.json');
    let metals = load('./src/datagen/data/metallum_metals.json');
    let toolMetals = metals.filter(metal => metal.hasToolArmor);
    let ores = load('./src/datagen/data/ores.json');
    let stones = load('./src/datagen/data/stones.json');
    let oreType = ['poor', 'rich', 'normal'];
    let barMetals = MetallumMetals.filter(m => ['enderium', 'titanium', 'tungsten', 'tungsten_steel'].includes(m.name));
    let bellMetals = MetallumMetals.filter(m => ['beryllium_copper', 'florentine_bronze'].includes(m.name));
    let models = MetallumModelManager.create(modid);
    data.cluster_vein('normal_bauxite', 40, -32, 60, 20, 15, 'tfc_metallum:small_ore/bauxite', (rock) => {
        return [
